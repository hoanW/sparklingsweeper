...
export const CellView = ({ game, x, y, cell }: CellViewProps) => {
  ...
  const bomb = bombVisible ? <image selectable={false} url={'head.png'} imageWidth={30} imageHeight={30} />: <></>;

  const flagVisible = (cell.flagged && !game.gameOver && !game.paused) || (cell.bomb && game.winner);
  const flag = flagVisible ? <image selectable={false} url={'head.png'} imageWidth={30} imageHeight={30} /> : <></>;
